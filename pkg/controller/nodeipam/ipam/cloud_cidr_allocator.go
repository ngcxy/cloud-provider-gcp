...
func (ca *cloudCIDRAllocator) updateCIDRAllocation(nodeName string) error {
	...
	// In the case of DraNet and customized multi-NIC where a cluster with multi-networking
	// disabled but contains multi-NIC nodes, IPAM only executes the normal logic on
	// the default network interface and ignores any additional network interfaces.
